...
- 项目工作区采用“预测配置—计算表格—项目报表”三步结构；P1B 开放固定月金额、逐月填写、项目参数和公式计算。
...
- 历史项目主数据统一使用 `datasetId: historical-project-config-v2`；事实必须由历史配置计算生成。
...
- 税率、账期自动现金流、多场景、版本快照和 Excel 属于后续阶段。
- 历史项目必须先保存配置、再通过 `CalculationService` 生成事实；禁止恢复直接写入“结构验证事实”的做法。
- 历史项目核对以 `docs/reports/02_P1A_历史项目回放与结果核对报告.md` 和 `docs/reports/03_P1B_参数与公式验收报告.md` 为基线；源表冲突必须记录，不得用隐藏调整项强行对齐。
