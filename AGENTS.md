...
- `docs/03_原型/01_当前原型_项目中心与预测工作台.html` 只保留为早期结构参考；当前正式外壳不设全局顶栏，左侧导航常驻并可折叠，产品标识和折叠按钮位于侧栏顶部，数据库状态和备份恢复位于侧栏底部。平台根页面只显示统一高度标题栏，不显示重复面包屑；面包屑只用于项目详情层级。
...
- 外层提供“项目列表、主数据管理、指标管理”；项目列表默认展示测算中项目，并在同一列表通过“查看归档（数量）”切换归档视图，不设置独立归档目录；进入项目后左侧导航和应用外壳保持不变，只在右侧切换为项目工作区。
