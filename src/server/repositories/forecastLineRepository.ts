...
export class ForecastLineRepository {
  ...
  async saveProjectDraft(projectId: string, planId: string, drafts: ForecastLineDraft[]) {
    ...
    const statements: SqlStatement[] = removed.map((item) => ({
      sql: 'DELETE FROM cfg_model_line WHERE id = ? AND project_id = ? AND plan_id = ?', params: [item.id, projectId, planId],
    }))
